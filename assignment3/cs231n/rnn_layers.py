...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  c = np.tanh(next_c)
  next_h = o * c
  cache = (x, prev_h, prev_c, Wx, Wh, i, f, o, g, c)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  pass
  _, H = dnext_h.shape
  x, prev_h, prev_c, Wx, Wh, i, f, o, g, c = cache
  dnext_c += dnext_h * o * (1 - c**2) #attention "+=" !
  do = dnext_h * c
  df = dnext_c * prev_c
  dprev_c = dnext_c * f #lstm advantage
  di = dnext_c * g
  dg = dnext_c * i
  dai = di * (i * (1 - i))
  daf = df * (f * (1 - f))
...
def lstm_backward(dh, cache):
  ...
  #############################################################################
  pass
  x, prev_h, prev_c, Wx, Wh, i, f, o, g, c = cache[-1]
  N, T, H = dh.shape
  N, D = x.shape
  dh0 = np.zeros((N, H))
  db = np.zeros(4*H)
