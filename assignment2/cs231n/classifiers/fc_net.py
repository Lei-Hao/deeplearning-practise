...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    out, cache = affine_forward(hidden_out, self.params['W2'], self.params['b2'])
    scores = out
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dx2, dw2, db2 = affine_backward(dout, cache)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dx, dw, db = affine_backward(dout, ar_cache[self.num_layers-1])
    grads['W%d'%(self.num_layers)] = dw + self.reg * self.params['W%d'%(self.num_layers)]
    grads['b%d'%(self.num_layers)] = db
    dhout = dx

    for idx in xrange(self.num_layers-1):
        lay = self.num_layers-1-idx
        loss += 0.5 * self.reg * np.sum(self.params['W%d'%(lay)] * self.params['W%d'%(lay)])
        if self.use_dropout:
            dhout = dropout_backward(dhout, dp_cache[lay-1])
        if self.use_batchnorm:
            dx, dw, db, dgamma, dbeta = affine_bn_relu_backward(dhout, ar_cache[lay-1])
        else:
            dx, dw, db = affine_relu_backward(dhout, ar_cache[lay-1])
