...
def batchnorm_backward(dout, cache):
  ...
  #############################################################################
  pass
  (x, x_mean, x_var, x_normalized, gamma, eps) = cache
  N = x.shape[0]
...
"""
  ...
  for i in xrange(H_out):
     for j in xrange(W_out):
        ...
        temp_binary_mask = (x_masked == max_x_masked.reshape(N, C, 1, 1))
        dx[:,:,i*stride : i*stride+HH, j*stride : j*stride+WW] = temp_binary_mask * dout[:,:,i,j].reshape(N, C, 1, 1)
...
def svm_loss(x, y):
  ...
  #print correct_class_scores.shape #(50L,)
  #print correct_class_scores[:, np.newaxis].shape #(50L, 1L)
  #print correct_class_scores[np.newaxis, :].shape #(1L, 50L)
